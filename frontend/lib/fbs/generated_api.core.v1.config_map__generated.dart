...
library api.core.v1.config_map_;
...
class DataEntry {
  DataEntry._(this._bc, this._bcOffset);
  factory DataEntry(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<DataEntry> reader = _DataEntryReader();
...
class _DataEntryReader extends fb.TableReader<DataEntry> {
  const _DataEntryReader();

  @override
  DataEntry createObject(fb.BufferContext bc, int offset) => 
    DataEntry._(bc, offset);
}

class DataEntryBuilder {
  DataEntryBuilder(this.fbBuilder);
...
class DataEntryObjectBuilder extends fb.ObjectBuilder {
  final String? _key;
  final String? _value;

  DataEntryObjectBuilder({
    String? key,
    String? value,
  })
      : _key = key,
...
class BinaryDataEntry {
  BinaryDataEntry._(this._bc, this._bcOffset);
  factory BinaryDataEntry(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<BinaryDataEntry> reader = _BinaryDataEntryReader();
  ...
  @override
  String toString() {
    return 'BinaryDataEntry{key: ${key}, value: ${value}}';
  }
}

class _BinaryDataEntryReader extends fb.TableReader<BinaryDataEntry> {
  const _BinaryDataEntryReader();

  @override
  BinaryDataEntry createObject(fb.BufferContext bc, int offset) => 
    BinaryDataEntry._(bc, offset);
}

class BinaryDataEntryBuilder {
  BinaryDataEntryBuilder(this.fbBuilder);
...
class BinaryDataEntryObjectBuilder extends fb.ObjectBuilder {
  final String? _key;
  final List<int>? _value;

  BinaryDataEntryObjectBuilder({
    String? key,
    List<int>? value,
  })
      : _key = key,
