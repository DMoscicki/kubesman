...
library api.core.v1.node_status_;
...
class AllocatableEntry {
  AllocatableEntry._(this._bc, this._bcOffset);
  factory AllocatableEntry(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<AllocatableEntry> reader = _AllocatableEntryReader();
  ...
  @override
  String toString() {
    return 'AllocatableEntry{key: ${key}, value: ${value}}';
  }
}

class _AllocatableEntryReader extends fb.TableReader<AllocatableEntry> {
  const _AllocatableEntryReader();

  @override
  AllocatableEntry createObject(fb.BufferContext bc, int offset) => 
    AllocatableEntry._(bc, offset);
}

class AllocatableEntryBuilder {
  AllocatableEntryBuilder(this.fbBuilder);
...
class AllocatableEntryObjectBuilder extends fb.ObjectBuilder {
  final String? _key;
  final apimachinery_pkg_api_resource.QuantityObjectBuilder? _value;

  AllocatableEntryObjectBuilder({
    String? key,
    apimachinery_pkg_api_resource.QuantityObjectBuilder? value,
  })
      : _key = key,
