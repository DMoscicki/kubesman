...
class OverheadEntry {
  OverheadEntry._(this._bc, this._bcOffset);
  factory OverheadEntry(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<OverheadEntry> reader = _OverheadEntryReader();
  ...
  @override
  String toString() {
    return 'OverheadEntry{key: ${key}, value: ${value}}';
  }
}

class _OverheadEntryReader extends fb.TableReader<OverheadEntry> {
  const _OverheadEntryReader();

  @override
  OverheadEntry createObject(fb.BufferContext bc, int offset) => 
    OverheadEntry._(bc, offset);
}

class OverheadEntryBuilder {
  OverheadEntryBuilder(this.fbBuilder);
...
class OverheadEntryObjectBuilder extends fb.ObjectBuilder {
  final String? _key;
  final apimachinery_pkg_api_resource.QuantityObjectBuilder? _value;

  OverheadEntryObjectBuilder({
    String? key,
