...
library api.core.v1.resource_quota_status_;
...
class HardEntry {
  HardEntry._(this._bc, this._bcOffset);
  factory HardEntry(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<HardEntry> reader = _HardEntryReader();
  ...
  @override
  String toString() {
    return 'HardEntry{key: ${key}, value: ${value}}';
  }
}

class _HardEntryReader extends fb.TableReader<HardEntry> {
  const _HardEntryReader();

  @override
  HardEntry createObject(fb.BufferContext bc, int offset) => 
    HardEntry._(bc, offset);
}

class HardEntryBuilder {
  HardEntryBuilder(this.fbBuilder);
...
class HardEntryObjectBuilder extends fb.ObjectBuilder {
  final String? _key;
  final apimachinery_pkg_api_resource.QuantityObjectBuilder? _value;

  HardEntryObjectBuilder({
    String? key,
...
class UsedEntry {
  UsedEntry._(this._bc, this._bcOffset);
  factory UsedEntry(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<UsedEntry> reader = _UsedEntryReader();
  ...
  @override
  String toString() {
    return 'UsedEntry{key: ${key}, value: ${value}}';
  }
}

class _UsedEntryReader extends fb.TableReader<UsedEntry> {
  const _UsedEntryReader();

  @override
  UsedEntry createObject(fb.BufferContext bc, int offset) => 
    UsedEntry._(bc, offset);
}

class UsedEntryBuilder {
  UsedEntryBuilder(this.fbBuilder);
...
class UsedEntryObjectBuilder extends fb.ObjectBuilder {
  final String? _key;
  final apimachinery_pkg_api_resource.QuantityObjectBuilder? _value;

  UsedEntryObjectBuilder({
    String? key,
    apimachinery_pkg_api_resource.QuantityObjectBuilder? value,
  })
      : _key = key,
