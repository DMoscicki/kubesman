...
class StringDataEntry {
  StringDataEntry._(this._bc, this._bcOffset);
  factory StringDataEntry(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<StringDataEntry> reader = _StringDataEntryReader();
  ...
  @override
  String toString() {
    return 'StringDataEntry{key: ${key}, value: ${value}}';
  }
}

class _StringDataEntryReader extends fb.TableReader<StringDataEntry> {
  const _StringDataEntryReader();

  @override
  StringDataEntry createObject(fb.BufferContext bc, int offset) => 
    StringDataEntry._(bc, offset);
}

class StringDataEntryBuilder {
  StringDataEntryBuilder(this.fbBuilder);
...
class StringDataEntryObjectBuilder extends fb.ObjectBuilder {
  final String? _key;
  final String? _value;

  StringDataEntryObjectBuilder({
    String? key,
    String? value,
  })
      : _key = key,
