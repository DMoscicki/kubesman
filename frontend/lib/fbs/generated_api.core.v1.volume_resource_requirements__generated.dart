...
library api.core.v1.volume_resource_requirements_;
...
class LimitsEntry {
  LimitsEntry._(this._bc, this._bcOffset);
  factory LimitsEntry(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<LimitsEntry> reader = _LimitsEntryReader();
  ...
  @override
  String toString() {
    return 'LimitsEntry{key: ${key}, value: ${value}}';
  }
}

class _LimitsEntryReader extends fb.TableReader<LimitsEntry> {
  const _LimitsEntryReader();

  @override
  LimitsEntry createObject(fb.BufferContext bc, int offset) => 
    LimitsEntry._(bc, offset);
}

class LimitsEntryBuilder {
  LimitsEntryBuilder(this.fbBuilder);
...
class LimitsEntryObjectBuilder extends fb.ObjectBuilder {
  final String? _key;
  final apimachinery_pkg_api_resource.QuantityObjectBuilder? _value;

  LimitsEntryObjectBuilder({
    String? key,
...
class RequestsEntry {
  RequestsEntry._(this._bc, this._bcOffset);
  factory RequestsEntry(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<RequestsEntry> reader = _RequestsEntryReader();
  ...
  @override
  String toString() {
    return 'RequestsEntry{key: ${key}, value: ${value}}';
  }
}

class _RequestsEntryReader extends fb.TableReader<RequestsEntry> {
  const _RequestsEntryReader();

  @override
  RequestsEntry createObject(fb.BufferContext bc, int offset) => 
    RequestsEntry._(bc, offset);
}

class RequestsEntryBuilder {
  RequestsEntryBuilder(this.fbBuilder);
...
class RequestsEntryObjectBuilder extends fb.ObjectBuilder {
  final String? _key;
  final apimachinery_pkg_api_resource.QuantityObjectBuilder? _value;

  RequestsEntryObjectBuilder({
    String? key,
    apimachinery_pkg_api_resource.QuantityObjectBuilder? value,
  })
      : _key = key,
