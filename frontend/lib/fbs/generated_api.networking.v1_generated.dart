...
class IngressBackend {
  ...
  IngressServiceBackend? get service => IngressServiceBackend.reader.vTableGetNullable(_bc, _bcOffset, 4);
  api_core_v1.TypedLocalObjectReference? get resource => api_core_v1.TypedLocalObjectReference.reader.vTableGetNullable(_bc, _bcOffset, 6);

  @override
  String toString() {
    return 'IngressBackend{service: ${service}, resource: ${resource}}';
  }
}
...
class IngressServiceBackend {
  IngressServiceBackend._(this._bc, this._bcOffset);
  factory IngressServiceBackend(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<IngressServiceBackend> reader = _IngressServiceBackendReader();

  final fb.BufferContext _bc;
  final int _bcOffset;

  String? get name => const fb.StringReader().vTableGetNullable(_bc, _bcOffset, 4);
  ServiceBackendPort? get port => ServiceBackendPort.reader.vTableGetNullable(_bc, _bcOffset, 6);

  @override
  String toString() {
    return 'IngressServiceBackend{name: ${name}, port: ${port}}';
  }
}

class _IngressServiceBackendReader extends fb.TableReader<IngressServiceBackend> {
  const _IngressServiceBackendReader();

  @override
  IngressServiceBackend createObject(fb.BufferContext bc, int offset) => 
    IngressServiceBackend._(bc, offset);
}

class IngressServiceBackendBuilder {
  IngressServiceBackendBuilder(this.fbBuilder);
...
class IngressServiceBackendObjectBuilder extends fb.ObjectBuilder {
  final String? _name;
  final ServiceBackendPortObjectBuilder? _port;

  IngressServiceBackendObjectBuilder({
    String? name,
    ServiceBackendPortObjectBuilder? port,
  })
      : _name = name,
        _port = port;
...
class IngressSpec {
  ...
  IngressBackend? get defaultBackend => IngressBackend.reader.vTableGetNullable(_bc, _bcOffset, 6);
  ...
  @override
  String toString() {
    return 'IngressSpec{ingressClassName: ${ingressClassName}, defaultBackend: ${defaultBackend}, tls: ${tls}, rules: ${rules}}';
  }
}
...
class IngressSpecBuilder {
  ...
  int addDefaultBackendOffset(int? offset) {
    fbBuilder.addOffset(1, offset);
    return fbBuilder.offset;
  }
...
class IngressSpecObjectBuilder extends fb.ObjectBuilder {
  ...
  int finish(fb.Builder fbBuilder) {
    ...
    final int? defaultBackendOffset = _defaultBackend?.getOrCreateOffset(fbBuilder);
    ...
    fbBuilder.addOffset(1, defaultBackendOffset);
...
class ServiceBackendPort {
  ServiceBackendPort._(this._bc, this._bcOffset);
  factory ServiceBackendPort(List<int> bytes) {
    final rootRef = fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<ServiceBackendPort> reader = _ServiceBackendPortReader();
  ...
  @override
  String toString() {
    return 'ServiceBackendPort{name: ${name}, number: ${number}}';
  }
}

class _ServiceBackendPortReader extends fb.TableReader<ServiceBackendPort> {
  const _ServiceBackendPortReader();

  @override
  ServiceBackendPort createObject(fb.BufferContext bc, int offset) => 
    ServiceBackendPort._(bc, offset);
}

class ServiceBackendPortBuilder {
  ServiceBackendPortBuilder(this.fbBuilder);
...
class ServiceBackendPortObjectBuilder extends fb.ObjectBuilder {
  final String? _name;
  final int? _number;

  ServiceBackendPortObjectBuilder({
    String? name,
    int? number,
  })
      : _name = name,
