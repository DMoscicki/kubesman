...
flatbufgen-from-proto:
    #!/usr/bin/env bash
    set -exuo pipefail
    fd -e proto -x sd 'map<string, bytes>' 'map<string, [ubyte]>'
    while read -r line; do
        flatc -I k8s-pb/protos -o k8s-fbs/src/${line%/*} --keep-proto-id --gen-all --rust-serialize --rust-module-root-file --proto k8s-pb/${line} --rust ;
...
flatbuffers: protos flatbufgen-from-proto
